...
export const useWithdrawToHosts = () => {
  ...
  const withdrawToHosts = async (llmId: number | string | bigint) => {
    await writeContract({
      abi: ABI as any,
  ...
  return { withdrawToHosts, txHash, isWriting, writeError, resetWrite, receipt, isConfirming, isConfirmed } as const;
};

export default useWithdrawToHosts;
