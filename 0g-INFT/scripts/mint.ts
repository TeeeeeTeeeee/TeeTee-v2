...
function loadStorageData(): StorageData {
  ...
  console.log('💾 Loaded Phase 1 storage data:');
...
function validateMintParams(params: MintParams): void {
  ...
  console.log('✅ Mint parameters validated');
}
...
async function mintINFT(
  ...
  console.log('🏭 Connecting to INFT contract...');
  ...
  console.log('🎯 Minting INFT with parameters:');
...
async function verifyMintedToken(
  ...
  console.log('🔍 Verifying minted token...');
 ...
 */
async function main() {
  console.log('🚀 Starting Phase 4 - Mint INFT');
  ...
  try {
    ...
    console.log('🔗 Token is now live on 0G Galileo testnet');
    console.log('📦 Encrypted AI agent data is bound to INFT Token ID:', mintResult.tokenId);
...
async function mintWithCustomParams(to?: string, encryptedURI?: string, metadataHash?: string) {
  console.log('🚀 Starting Custom INFT Mint');
  console.log('=' .repeat(60));
  ...
  try {
    ...
    if (encryptedURI && metadataHash) {
      ...
      console.log('📝 Using custom mint parameters');
    } else {
      ...
      console.log('📝 Using Phase 1 storage data with custom recipient');
    }
    ...
    console.log('\n✅ Custom mint completed successfully!');
    return mintResult;
    
  } catch (error) {
    console.error('❌ Error during custom minting:', error);
    throw error;
  }
}
